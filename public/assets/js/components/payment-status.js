...
function qrCodeContent(qrImageSrc) {
  if (!qrImageSrc) return "";
  const safeSrc = escapeHtml(qrImageSrc);
  return `<div class="rp-payment__qr-desktop"><span>Escaneie para pagar</span><div class="rp-payment__qr-frame"><img src="${safeSrc}" alt="QR Code Pix para pagamento" /></div></div><details class="rp-payment__qr-mobile"><summary>Mostrar QR Code</summary><div class="rp-payment__qr-frame"><img src="${safeSrc}" alt="QR Code Pix para pagamento" /></div></details>`;
}
...
export function renderPaymentStatus(order = {}) {
  ...
  const loading = order.phase === "creating",
    ...
    qrImageSrc = pixQrImageSrc(order),
