...
export function renderPaymentStatus(order = {}) {
  ...
  const total = paymentTotalCents(order);
  const qrCode = pixCode(order);
  const reference = shortOrderReference(paymentReference(order));
  const expiresAt = formatPixExpiry(order.data?.pix_expira_em);
  ...
  return `
    <div class="rp-payment" data-payment-root>
      ...
      <section class="rp-payment__sheet" role="dialog" aria-modal="true" aria-live="polite">
        ...
        ${failed ? `<p class="rp-kicker">Não deu certo</p><h2>Não conseguimos concluir o pagamento</h2><p>${esc(order.error || "Tente novamente em instantes.")}</p><button class="rp-btn rp-btn--primary" type="button" data-retry-payment>Tentar novamente</button><button class="rp-btn" type="button" data-close-payment>Voltar ao pedido</button>` : ""}
      </section>
    </div>
  `;
}
