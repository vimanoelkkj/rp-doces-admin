(() => {
  ...
  if (!cta) return;
  const destino = document.getElementById('menu-grid-bolo')?.closest('section') ||
                  document.getElementById('menu-grid-bolo');
  if (!destino) { cta.remove(); return; }
  ...
  function atualizar(){
    ...
    const passouHero = scrollY > Math.min(innerHeight * .55, 420);
    const estaNoCardapio = r.top < innerHeight * .72 && r.bottom > 90;
  ...
  cta.addEventListener('click', () => destino.scrollIntoView({behavior:'smooth',block:'start'}));
