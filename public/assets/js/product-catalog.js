...
async function carregarCardapioDinamico(){
  try {
    ...
    if (!boloGrid || !pudimGrid || !featuredGrid || !featuredSection || !emptyState || !filters || !menuSection) return;
    ...
    function controleCarrinho(produto){
      ...
      if(quantidade>0)return `<div class="rp-card-qty"><button type="button" data-card-cart-action="-1" aria-label="Remover uma unidade">−</button><strong>${quantidade}</strong><button type="button" data-card-cart-action="1" aria-label="Adicionar uma unidade" ${quantidade>=Number(produto.estoque)?'disabled':''}>+</button></div>`;
    ...
    function card(produto, revelar=false){
      ...
      div.dataset.precoOriginalCentavos = String(produto.preco_original_centavos ?? produto.preco_centavos ?? 0);
    ...
    function atualizarControlesCarrinho(){menuSection.querySelectorAll('.flavor-card').forEach(item=>{if(item._rpProduto){const target=item.querySelector('.rp-card-cart');if(target)target.innerHTML=controleCarrinho(item._rpProduto)}})}
    ...
    function preencherSecao(grid,lista,chave,botao){
      ...
      grid.replaceChildren(...visiveis.map((produto,indice)=>card(produto,expandida&&indice>=limiteInicial)));
      if(botao){
    ...
    function renderizar(filtro='TODOS'){
      ...
      const filtrados=produtos.filter(produto=>filtro==='TODOS'||produto.categoria===filtro||(filtro==='PROMOCOES'&&produto.promocao_vigente));
      ...
      const boloCategory=boloGrid.closest('[data-menu-category]'),pudimCategory=pudimGrid.closest('[data-menu-category]');
      ...
      boloGrid.classList.remove('is-loading');preencherSecao(boloGrid,bolos,'BOLO_NO_POTE',boloButton);boloCategory.hidden=!bolos.length;
      pudimGrid.classList.remove('is-loading');preencherSecao(pudimGrid,pudins,'MINI_PUDIM',pudimButton);pudimCategory.hidden=!pudins.length;
      preencherSecao(featuredGrid,destaques,'DESTAQUES',featuredButton);featuredSection.hidden=!destaques.length;
      emptyState.hidden=filtrados.length>0;
      emptyState.textContent=filtro==='PROMOCOES'?'Nenhuma promoção disponível no momento.':'Nenhum produto disponível neste filtro no momento.';
      filters.querySelectorAll('[data-menu-filter]').forEach(button=>button.classList.toggle('active',button.dataset.menuFilter===filtro));
    }
    filters.onclick=event=>{const button=event.target.closest('[data-menu-filter]');if(button)renderizar(button.dataset.menuFilter)};
