(() => {
  ...
  function destaque(card) {
    ...
    return (
      ...
      !!card.querySelector("[data-destaque],.badge-destaque,.destaque")
    );
  }
  ...
  function nome(card) {
    return (
      texto(card.querySelector("h3,h4,.product-name,.produto-nome,.card-title")) || texto(card)
    );
  }
  ...
  function enhanceTrack(track) {
    ...
    wrap.className = "rp-track-wrap";
    track.parentNode.insertBefore(wrap, track);
    wrap.appendChild(track);
    const prev = document.createElement("button"),
      next = document.createElement("button");
    prev.className = "rp-track-arrow prev";
    next.className = "rp-track-arrow next";
    prev.type = next.type = "button";
    prev.textContent = "‹";
    next.textContent = "›";
    prev.setAttribute("aria-label", "Produtos anteriores");
    next.setAttribute("aria-label", "Próximos produtos");
    wrap.append(prev, next);
    const move = d => track.scrollBy({ left: d * track.clientWidth * 0.82, behavior: "smooth" });
    prev.onclick = () => move(-1);
    next.onclick = () => move(1);
    const state = () => {
  ...
  function makeHeader(track, title, filter) {
    ...
    head.className = "rp-vitrine-head";
    const h = document.createElement("h3");
    h.textContent = title;
    const b = document.createElement("button");
    b.type = "button";
    b.className = "rp-see-all";
  ...
  async function openAll(title, filter) {
    ...
    overlay.querySelector("#rpAllTitle").textContent = title;
    overlay.classList.add("open");
    document.body.style.overflow = "hidden";
    overlay.querySelector("#rpAllSearch").value = "";
    overlay
      .querySelectorAll(".rp-filter")
      .forEach(x => x.classList.toggle("active", x.dataset.f === "todos"));
  ...
  function ensureOverlay() {
    ...
    o.addEventListener("click", e => {
      if (e.target === o) fecharCatalogo();
    });
    o.querySelector("#rpAllSearch").addEventListener("input", renderAll);
    o.querySelectorAll(".rp-filter").forEach(
      b =>
        (b.onclick = () => {
          o.querySelectorAll(".rp-filter").forEach(x => x.classList.remove("active"));
  ...
  function rebuild() {
    ...
    if (cards.length) {
      ...
      const parents = [...new Set(cards.map(c => c.parentElement))].filter(
        p =>
          p &&
  ...
  if (document.readyState === "loading")
    document.addEventListener("DOMContentLoaded", () => setTimeout(rebuild, 50), { once: true });
  else setTimeout(rebuild, 50);
