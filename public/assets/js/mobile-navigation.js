(() => {
  ...
  function atualizarSecaoAtiva() {
    if (
      ...
      document.body.classList.contains("rp-modal-open")
    )
      return;
    const ponto = innerHeight * 0.45;
    const sabores = document.getElementById("sabores")?.getBoundingClientRect();
    const secoesMenu = ["sobre", "onde-encontrar", "contato"]
      .map(id => document.getElementById(id)?.getBoundingClientRect())
      .filter(Boolean);
    limparAtivos();
    if (sabores && sabores.top <= ponto && sabores.bottom >= ponto)
      flavorsButton?.classList.add("active");
    else if (secoesMenu.some(secao => secao.top <= ponto && secao.bottom >= ponto))
  ...
  function liberarPagina() {
    if (!menuOverlay.classList.contains("open") && !partyOverlay.classList.contains("open"))
  ...
  function abrirFestas() {
    ...
    bloquearPagina();
    partyClose.focus({ preventScroll: true });
  }
  function fecharFestas() {
    partyOverlay.classList.remove("open");
    partyOverlay.setAttribute("aria-hidden", "true");
    limparAtivos();
    document.body.classList.remove("rp-party-open");
    partySheet.style.transform = "";
    liberarPagina();
    atualizarSecaoAtiva();
  }

  menuButton.addEventListener("click", () =>
    menuOverlay.classList.contains("open") ? fecharMenu() : trocarPainel(abrirMenu)
  );
  partyButton?.addEventListener("click", () =>
    partyOverlay.classList.contains("open") ? fecharFestas() : trocarPainel(abrirFestas)
  );
  ...
  partyClose.addEventListener("click", fecharFestas);
  menuOverlay.addEventListener("click", event => {
    if (event.target === menuOverlay) fecharMenu();
  });
  partyOverlay.addEventListener("click", event => {
    if (event.target === partyOverlay) fecharFestas();
  });
  document
    .querySelectorAll("[data-mobile-menu-link]")
    .forEach(link => link.addEventListener("click", fecharMenu));
  document
    .querySelector("[data-mobile-party]")
    ?.addEventListener("click", () => trocarPainel(abrirFestas));
  flavorsButton?.addEventListener("click", () =>
    trocarPainel(() => {
  ...
  addEventListener("resize", () => {
    if (innerWidth > 700) {
      if (menuOverlay.classList.contains("open")) fecharMenu();
      if (partyOverlay.classList.contains("open")) fecharFestas();
    } else atualizarSecaoAtiva();
  });

  function habilitarArraste(sheet, fechar) {
    const handle = sheet.querySelector(".rp-mobile-menu-handle");
    let inicio = 0,
      distancia = 0,
      ponteiro = null;
    if (!handle) return;
    handle.addEventListener("pointerdown", event => {
      ...
      distancia = 0;
      handle.setPointerCapture?.(ponteiro);
    ...
    const terminar = event => {
      if (event.pointerId !== ponteiro) return;
      handle.releasePointerCapture?.(ponteiro);
      ponteiro = null;
      sheet.style.transition = "";
      if (distancia > 70) fechar();
  ...
  habilitarArraste(menuSheet, fecharMenu);
  habilitarArraste(partySheet, fecharFestas);

  let ocasiao = "Aniversário";
  partyOptions?.addEventListener("click", event => {
    const option = event.target.closest("[data-party-occasion]");
    if (!option) return;
    ocasiao = option.dataset.partyOccasion;
    partyOptions
      .querySelectorAll(".rp-party-option")
  ...
  function falarSobreFesta() {
    ...
    if (typeof window.abrirWhatsApp === "function") window.abrirWhatsApp(texto);
      ...
      window.open(
        `https://wa.me/${String(window.RP_WHATSAPP_NUMBER || "5533991285907").replace(/\D/g, "")}?text=${encodeURIComponent(texto)}`,
        "_blank",
        "noopener,noreferrer"
      );
  }
