...
function humanOrderStatus(status) {
  ...
  const labels = { NOVO: "Novo", EM_PREPARO: "Em preparo", "EM PREPARO": "Em preparo", PREPARANDO: "Em preparo", PRONTO: "Pronto", CONCLUIDO: "Concluído", CONCLUÍDO: "Concluído", CANCELADO: "Cancelado", ARQUIVADO: "Arquivado" };
...
function humanPaymentStatus(status) {
  ...
  const labels = { PAGO: "Pago", PENDENTE: "Aguardando pagamento", CANCELADO: "Cancelado", FALHA: "Falha", REEMBOLSADO: "Reembolsado" };
...
function recentOrdersMarkup(orders) {
  ...
  return `<div class="dashboard-orders"><div class="dashboard-orders__head" aria-hidden="true"><span>Pedido</span><span>Cliente</span><span>Itens</span><span>Pagamento</span><span>Andamento</span><span>Total</span></div>${orders.map(order => {
    ...
    return `<article class="dashboard-order"><div class="dashboard-order__id" data-label="Pedido"><strong>${reference}</strong><span>${time(order.criado_em)}</span></div><div class="dashboard-order__customer" data-label="Cliente"><span class="dashboard-customer-avatar" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="12" cy="8.4" r="3.1"></circle><path d="M6.2 18.6c.9-2.9 3-4.5 5.8-4.5s4.9 1.6 5.8 4.5"></path></svg></span><span><strong>${escapeHtml(order.cliente_nome || "Cliente")}</strong><small>${escapeHtml(order.cliente_whatsapp || order.cliente_email || "")}</small></span></div><div data-label="Itens">${itemCount} ${itemCount === 1 ? "item" : "itens"}</div><div data-label="Pagamento"><span class="status-pill" data-tone="${paymentTone(order.status_pagamento)}">${escapeHtml(humanPaymentStatus(order.status_pagamento))}</span></div><div data-label="Andamento"><span class="status-pill" data-tone="${orderTone(order.status_pedido)}">${escapeHtml(humanOrderStatus(order.status_pedido))}</span></div><div class="dashboard-order__total" data-label="Total">${money(order.valor_total_centavos)}</div></article>`;
  }).join("")}</div>`;
}
...
function dashboardMarkup({ orders, products }) {
  ...
  const paidToday = orders.filter(order => String(order.status_pagamento).toUpperCase() === "PAGO" && sameLocalDay(parseDate(order.pago_em || order.atualizado_em), now));
  const pendingPayment = orders.filter(order => String(order.status_pagamento).toUpperCase() === "PENDENTE");
  const waitingPreparation = orders.filter(order => String(order.status_pedido || "NOVO").toUpperCase() === "NOVO" && String(order.status_pagamento || "").toUpperCase() === "PAGO");
  ...
  return `<section class="dashboard" aria-label="Resumo da operação"><div class="dashboard-metrics">${metric("Faturamento pago hoje", money(paidRevenue), `${paidToday.length} pedido${paidToday.length === 1 ? " pago" : "s pagos"}`, "brand", true)}${metric("Aguardando preparação", String(waitingPreparation.length), "Pedidos pagos que ainda estão novos", "warning")}${metric("Aguardando pagamento", String(pendingPayment.length), "Pix ainda não confirmado", "warning")}${metric("Catálogo", String(products.length), `${soldOut.length} esgotado${soldOut.length === 1 ? "" : "s"} · ${lowStock.length} estoque baixo`, soldOut.length || lowStock.length ? "warning" : "success")}${metric("Pedidos hoje", String(today.length), "Criados desde 00:00", "default")}</div><div class="dashboard-grid"><section class="admin-panel dashboard-section dashboard-section--orders"><header class="dashboard-section__head"><div><strong>Pedidos recentes</strong><span>Últimas movimentações da loja</span></div><button type="button" class="dashboard-link" data-go-page="pedidos">Ver pedidos</button></header>${recentOrdersMarkup(recent)}</section><aside class="admin-panel dashboard-section dashboard-attention"><header class="dashboard-section__head"><div><strong>Precisa de atenção</strong><span>Pontos que podem exigir uma ação</span></div></header>${attention.length ? `<ul>${attention.map(item => `<li><span class="dashboard-attention__dot"></span>${escapeHtml(item)}</li>`).join("")}</ul>` : `<div class="dashboard-attention__ok"><span>✓</span><div><strong>Tudo tranquilo por aqui</strong><small>Nenhuma pendência operacional detectada.</small></div></div>`}</aside></div></section>`;
}
...
export async function renderDashboard(container, { onUnauthorized, onNavigate, isActive = () => true, force = false } = {}) {
  ...
  try {
    ...
    const data = { orders: ordersPayload?.pedidos || [], products: productsPayload?.produtos || [] };
