...
export async function onRequestPost({ request, env }) {
  ...
  if (Number(count?.total || 0) !== 0) return json({ erro: "Configuração inicial já realizada." }, 409);
  ...
  if (nome.length < 2 || !/^[a-z0-9._-]{3,30}$/.test(username) || !email.includes("@")) return json({ erro: "Nome, usuário ou e-mail inválido." }, 400);
