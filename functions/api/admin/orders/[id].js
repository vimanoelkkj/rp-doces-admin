...
export async function onRequestPut({ request, env, params }) {
  ...
  if (!Number.isInteger(id) || id < 1 || !VALIDOS.has(status)) return json({ erro: "Dados inválidos." }, 400);
  const existe = await env.DB.prepare("SELECT id, status_pagamento FROM pedidos WHERE id = ?").bind(id).first();
  if (!existe) return json({ erro: "Pedido não encontrado." }, 404);
  if (existe.status_pagamento !== "PAGO") return json({ erro: "O andamento só pode ser alterado após a confirmação do pagamento." }, 409);
