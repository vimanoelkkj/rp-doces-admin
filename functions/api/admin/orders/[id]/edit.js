...
async function cancelPendingMercadoPagoOrder(env, pedido) {
  ...
  const current = await syncOrderPayment(env, {
    pedidoId: pedido.id,
    order: currentOrder,
  ...
  if (current.status_pagamento === "PAGO") {
    return { ok: false, status: 409, erro: "O pagamento foi confirmado e este pedido não pode mais ser editado." };
  }

  if (current.status_pagamento === "CANCELADO") return { ok: true };
  if (current.status_pagamento !== "PENDENTE") {
    return { ok: false, status: 409, erro: "Este pagamento já foi encerrado e o pedido não pode mais ser editado." };
  }
  ...
  if (canceled.status_pagamento !== "CANCELADO") {
    return { ok: false, status: 409, erro: "O Mercado Pago não confirmou o cancelamento do Pix atual." };
  }
  return { ok: true };
}
...
export async function onRequestPut({ request, env, params }) {
  ...
  if (!requested || !ADMIN_PAYMENT_METHODS.has(method)) {
    return json({ erro: "Dados da edição inválidos." }, 400);
  }

  let pedido = await env.DB.prepare(
    `SELECT id, status_pagamento, status_pedido, reserva_status, mp_order_id
  ...
  if (pedido.status_pagamento !== "PENDENTE") {
    return json({ erro: "Somente pedidos com pagamento pendente podem ter itens e valor alterados." }, 409);
  }
  ...
  try {
    ...
    logEvent("warn", "admin_order.edit_payment_cancel_failed", {
      pedido_id: id,
  ...
  statements.push(
    env.DB.prepare(
      `UPDATE pedidos
       SET produto_id = ?, produto_nome = ?, quantidade = ?, valor_unitario_centavos = ?,
           valor_total_centavos = ?, metodo_pagamento = ?, status_pagamento = 'PENDENTE',
  ...
  logEvent("info", "admin_order.edited", {
    pedido_id: id,
    itens: items.length,
