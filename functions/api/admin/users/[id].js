...
async function getTarget(env, id) {
  return env.DB.prepare("SELECT id, ativo, papel FROM usuarios_admin WHERE id=? LIMIT 1").bind(id).first();
}
...
async function handlePut({ request, env, params }) {
  ...
  if (!Number.isInteger(id) || id < 1 || !d || typeof d !== "object" || Array.isArray(d)) return json({ erro: "Dados inválidos." }, 400);
  ...
  if (d.acao === "resetar_senha") {
    ...
    if (id !== Number(auth.user.id) && !isOwner(auth.user)) return json({ erro: "Sem permissão para redefinir esta senha." }, 403);
  ...
  if (d.acao === "toggle_ativo") {
    if (!isOwner(auth.user)) return json({ erro: "Apenas um administrador mestre pode alterar o estado de contas." }, 403);
    if (Object.keys(d).some((k) => !["acao", "ativo"].includes(k)) || typeof d.ativo !== "boolean") return json({ erro: "Estado da conta inválido." }, 400);
    if (id === Number(auth.user.id)) return json({ erro: "Você não pode alterar o estado da sua própria conta." }, 403);

    if (!d.ativo && target.papel === "OWNER") {
      const owners = await env.DB.prepare("SELECT COUNT(*) AS total FROM usuarios_admin WHERE papel='OWNER' AND ativo=1").first();
      if (Number(owners?.total || 0) <= 1) return json({ erro: "A loja precisa manter pelo menos um administrador mestre ativo." }, 409);
    }

    await env.DB.prepare("UPDATE usuarios_admin SET ativo=?, atualizado_em=CURRENT_TIMESTAMP WHERE id=?")
      .bind(d.ativo ? 1 : 0, id).run();
    if (!d.ativo) await env.DB.prepare("DELETE FROM admin_sessoes WHERE usuario_id=?").bind(id).run();
    return json({ ok: true });
  }

  if (d.acao === "alterar_papel") {
    if (!isOwner(auth.user)) return json({ erro: "Apenas um administrador mestre pode alterar níveis de acesso." }, 403);
    if (Object.keys(d).some((k) => !["acao", "papel"].includes(k)) || !["OWNER", "ADMIN"].includes(d.papel)) return json({ erro: "Nível de acesso inválido." }, 400);
    if (id === Number(auth.user.id)) return json({ erro: "Altere o nível da sua própria conta somente por outro administrador mestre." }, 403);
    ...
    await env.DB.batch([
      env.DB.prepare("UPDATE usuarios_admin SET papel=?, atualizado_em=CURRENT_TIMESTAMP WHERE id=?").bind(d.papel, id),
      env.DB.prepare("DELETE FROM admin_sessoes WHERE usuario_id=?").bind(id),
    ]);
    return json({ ok: true });
  }
