...
export async function onRequestPost({ request, env }) {
  ...
  const auth = await requireUser(env, request);
  if (auth.error) return auth.error;
  if (!isOwner(auth.user)) return json({ erro: "Apenas um administrador mestre pode criar administradores." }, 403);

  const d = await bodyJson(request);
  if (!d || typeof d !== "object" || Array.isArray(d)) return json({ erro: "Dados inválidos." }, 400);
  ...
  if (nome.length < 2 || nome.length > 100 || !/^[a-z0-9._-]{3,30}$/.test(username) || email.length > 254 || !email.includes("@") || !PAPEIS.has(papel) || err) {
  ...
  try {
    await env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, 1, ?)
    `).bind(nome, username, email, await hashPassword(senha), papel).run();
  } catch {
