...
async function finishRegistration(request, env, user, data) {
  ...
  try {
    ...
    return json(
      {
        erro: duplicate
          ? "Esta biometria já está cadastrada."
          : passkeyError(error, "Não foi possível cadastrar a biometria.")
      },
      duplicate ? 409 : 400
    );
  }
...
export async function onRequestDelete({ request, env }) {
  ...
  if (Number(result.meta?.changes || 0) !== 1)
    return json({ erro: "Credencial não encontrada." }, 404);
  return json({ ok: true });
}
