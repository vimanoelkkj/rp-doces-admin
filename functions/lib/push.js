...
async function vapidAuthorization(
  ...
  publicKey,
  privateKey,
  subject = "mailto:contato@rp-doces.pages.dev"
) {
...
async function encryptPayload(subscription, payload) {
  const uaPub = b64urlToBytes(subscription.p256dh),
...
export async function sendWebPush(env, subscription, payload) {
  ...
  const authorization = await vapidAuthorization(
    ...
    env.VAPID_SUBJECT || "mailto:contato@rp-doces.pages.dev"
  );
...
export async function notifyPaidOrder(env, pedidoId) {
  ...
  const subs =
    (await env.DB.prepare("SELECT id, endpoint, p256dh, auth FROM push_inscricoes").all())
      .results || [];
  if (!subs.length) return { ok: false, reason: "no_subscriptions" };
  const pedido = await env.DB.prepare(
    `SELECT id, cliente_nome, produto_nome, quantidade, valor_total_centavos, tipo_entrega, status_pagamento FROM pedidos WHERE id=? LIMIT 1`
  )
    .bind(pedidoId)
    .first();
  if (!pedido || pedido.status_pagamento !== "PAGO") return { ok: false, reason: "not_paid" };
  const claim = await env.DB.prepare("INSERT OR IGNORE INTO push_eventos (pedido_id) VALUES (?)")
    .bind(pedidoId)
    .run();
  if (!claim.meta?.changes) return { ok: true, reason: "already_sent" };
  const payload = {
    title: "🍰 Novo pedido pago!",
    body: `Pedido #${pedido.id} • ${pedido.cliente_nome || "Cliente"} • R$ ${(Number(pedido.valor_total_centavos || 0) / 100).toFixed(2).replace(".", ",")}`,
  ...
  for (const sub of subs) {
    try {
      ...
      if (r.ok) delivered++;
        ...
        await env.DB.prepare("DELETE FROM push_inscricoes WHERE id=?").bind(sub.id).run();
      else
  ...
  if (!delivered)
    await env.DB.prepare("DELETE FROM push_eventos WHERE pedido_id=?").bind(pedidoId).run();
